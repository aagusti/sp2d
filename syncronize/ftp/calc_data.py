...
print eis_year, eis_month, eis_day
#UPDATE DATA wells
#rows = Eis(None)
#print eis_year
eng_dst.echo =True
...
for row in rows:
    ...
    #print row.chart.chart_type, row.chart.label
    ...
    if row.chart.chart_type=='pie': 
        ...
        if row.chart.label[:3]=='JAN':
            ...
            for i in range(1,13):
                ...
                if i<10:
                    row_dict['value_%s' %i] = int(row_sum)
                else:
                    row_dict['value%s' %i] = int(row_sum)
            ...
            for i in range(7,13):
                ...
                for tup in tupKode:
                    ...
                    if row_data:
                        row_sum += row_data
                row_dict['value_%s' %str(i-6)] = int(row_sum)
            row.from_dict(row_dict)
