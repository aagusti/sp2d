...
def data_found(source):
    sql = text("""SELECT COUNT(*) C FROM apbd.ar_invoice_item
...
def insert(source):
    sql = text("""INSERT INTO apbd.ar_invoice_item(unit_id, kode, disabled, created, 
                  ...
                  VALUES(:unit_id, :kode, :disabled, :created, :updated, :create_uid, 
...
def update(source):
    sql = text("""UPDATE apbd.ar_invoice_item 
                    set unit_id        = :unit_id,
                        kode           = :kode,
                        disabled       = :disabled,
                        created        = :created,
...
for file in os.listdir('%s' % path):
    ...
    #file = 
    if fileExtension == '.csv' and fileName.find('KETETAPAN')>0:
